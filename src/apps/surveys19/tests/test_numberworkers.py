...
class NumberWorkersTestCase(TestCase):
    ...
    def test_create_numberworkers(self):
        content_type = ContentType.objects.get(app_label='surveys19', model='longtermhire')
        age_scope = AgeScope.objects.get(id=2)
        numberworkers_list_before_size = len(NumberWorkers.objects.all())

        #new value
        NumberWorkers.objects.create(count=20, content_type=content_type, object_id=3, age_scope=age_scope)

...
class ModelTestCase(TestCase):
    ...
    def test_create_business(self):
        ...
        farmer_land_type_b = FarmerLandType.objects.get(id=5)

        farmer_land_area_list_before_size = len(FarmerLandArea.objects.all())

        #new value
        FarmerLandArea.objects.create(survey=survey_id)
        new_farmer_land_area = FarmerLandArea.objects.get(survey=survey_id)
        new_farmer_land_area.farmer_land_type.add(farmer_land_type_a, farmer_land_type_b)
        new_farmer_land_area.save()

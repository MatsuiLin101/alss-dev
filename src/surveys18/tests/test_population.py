...
from surveys18.models import Survey, LifeStyle, FarmerWorkDay, OtherFarmWork, EducationLevel, Relationship, Gender
...
def setUp(self):
    ...
    call_command('loaddata', 'test/education-level.yaml', verbosity=0)
    call_command('loaddata', 'test/farmer-work-day.yaml', verbosity=0)
    call_command('loaddata', 'test/life-style.yaml', verbosity=0)
    call_command('loaddata', 'test/other-farm-work.yaml', verbosity=0)
    call_command('loaddata', 'test/relationship.yaml', verbosity=0)
    call_command('loaddata', 'test/education-level.yaml', verbosity=0)
    call_command('loaddata', 'test/gender.yaml', verbosity=0)

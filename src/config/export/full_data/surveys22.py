...
class SurveyRelationGeneratorFactory110:
    ...
    def __init__(self, readonly=False, filters={}, excludes={}):
        ...
        self.subsidy__applies = self.ExportRelation(self.apply_results_generate(), 1)
    ...
    def apply_results_generate(self):
        qs = self.get_queryset(
            Apply.objects.prefetch_related("result").all(), prefix="subsidy__survey__"
        )
        for obj in qs.iterator():
            yield (obj.result.name,)
    ...
    def export_generator(self):
        ...
        for survey in self.surveys:
            try:
                ...
                apply_results = self.get_sliced_relation_generator(
                    survey.farmer_id, "subsidy__applies"
                )
